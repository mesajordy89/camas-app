...
# ============================================================
#                 CONFIGURACIÓN GENERAL Y GITHUB
# ============================================================
...
# ============================================================
#            PERSISTENCIA EN GITHUB Y MANEJO DE DATOS
# ============================================================


def guardar_csv_github(df, nombre_archivo):
  """Hace commit del DataFrame directamente al repositorio de GitHub via REST API."""
  token = st.secrets.get("GITHUB_TOKEN", "")
  if not token:
    # Fallback local en caso de testing offline
    df.to_csv(nombre_archivo, index=False, encoding="utf-8-sig")
    return True

  url = f"https://api.github.com/repos/{REPO_GITHUB}/contents/{nombre_archivo}"
  headers = {
  ...
  payload = {
      "message": f"Actualización POS: {nombre_archivo}",
      "content": content_b64,
      "branch": "main",
  }
  if sha:
...
def normalizar_inventario(df_input):
  ...
  df["STOCK"] = (
      pd.to_numeric(df["STOCK"], errors="coerce").fillna(0).astype(int).clip(lower=0)
  )
  df["STOCK_MINIMO"] = (
      pd.to_numeric(df["STOCK_MINIMO"], errors="coerce")
      .fillna(1)
      .astype(int)
      .clip(lower=0)
  )
  df["PRECIO"] = (
      pd.to_numeric(df["PRECIO"], errors="coerce").fillna(0.0).clip(lower=0)
  )

  return df[df["CATEGORIA"] != ""].reset_index(drop=True)[
      ["CATEGORIA", "STOCK", "PRECIO", "STOCK_MINIMO"]
  ]
...
def normalizar_ventas(df_input):
  ...
  for columna in COLUMNAS_VENTAS:
    if columna not in df.columns:
      if columna == "ABONADO":
        df[columna] = (
            pd.to_numeric(df.get("TOTAL", 0), errors="coerce").fillna(0.0)
        )
...
def cargar_inventario():
  url_raw = f"https://raw.githubusercontent.com/{REPO_GITHUB}/main/{FILE_INV}?t={pd.Timestamp.now().timestamp()}"
  try:
    ...
    if os.path.exists(FILE_INV):
      ...
      df = pd.DataFrame([
          {
              "CATEGORIA": "CAMAS - CAMA TAPIZADA DE LUCES 2PLZ",
              "STOCK": 5,
              "PRECIO": 150.0,
              "STOCK_MINIMO": 2,
          },
          {
              "CATEGORIA": "COLCHONES - COLCHON SUEÑO TOTAL 2PLZS",
              "STOCK": 5,
              "PRECIO": 100.0,
              "STOCK_MINIMO": 2,
          },
...
def cargar_ventas():
  url_raw = f"https://raw.githubusercontent.com/{REPO_GITHUB}/main/{FILE_VENTAS}?t={pd.Timestamp.now().timestamp()}"
  try:
    df = pd.read_csv(url_raw, encoding="utf-8-sig")
  except Exception:
...
def generar_pdf_recibo(venta_dict):
  ...
  p.drawString(
      300,
      y,
      f"Saldo Pendiente: ${float(venta_dict.get('SALDO_PENDIENTE', 0)):,.2f}",
  )
...
def obtener_subproductos(df, principal):
  ...
  return df[
      df["CATEGORIA"].astype(str).str.startswith(prefijo, na=False)
  ].copy()
...
def obtener_productos_vendibles(df):
  ...
  return [
      nombre
      for nombre in df["CATEGORIA"].tolist()
      if producto_es_vendible(df, nombre)
  ]
...
def existe_producto(df, nombre):
  ...
  return (
      nombre.strip().lower()
      in df["CATEGORIA"].astype(str).str.strip().str.lower().values
  )
...
def agregar_combo_al_carrito(cama_nombre, colchon_nombre, precio_combo):
  ...
  stk_cama = (
      int(
          df_inv_local[df_inv_local["CATEGORIA"] == cama_nombre]["STOCK"].values[
              0
          ]
      )
      if cama_nombre in df_inv_local["CATEGORIA"].values
      else 0
  )
  stk_colchon = (
      int(
          df_inv_local[df_inv_local["CATEGORIA"] == colchon_nombre][
              "STOCK"
          ].values[0]
      )
      if colchon_nombre in df_inv_local["CATEGORIA"].values
      else 0
  )

  if stk_cama < 1 or stk_colchon < 1:
    st.error(
        "❌ No hay suficiente stock disponible de uno de los productos para"
        " armar el combo."
    )
    return False

  st.session_state["carrito"].append(
      {"producto": cama_nombre, "cantidad": 1, "precio": float(precio_combo / 2)}
  )
  st.session_state["carrito"].append({
...
dinero_recibido = (
    float(df_ventas["ABONADO"].sum()) if not df_ventas.empty else 0.0
)
...
r1.markdown(
    f'<div class="info-card">💰 Recaudado<br><b>${dinero_recibido:,.2f}</b></div>',
    unsafe_allow_html=True,
)
r2.markdown(
    f'<div class="info-card">📦 Stock Total<br><b>{total_stock} uds</b></div>',
    unsafe_allow_html=True,
)
r3.markdown(
    f'<div class="info-card">🧾 Ventas Totales<br><b>{total_operaciones}</b></div>',
    unsafe_allow_html=True,
)
r4.markdown(
    '<div class="info-card">⏳ Apartados'
    f" Pendientes<br><b>{total_apartados}</b></div>",
    unsafe_allow_html=True,
)
...
def abrir_modal_venta():
  ...
  if prod_sel:
    stk_max = int(
        df_inv_local[df_inv_local["CATEGORIA"] == prod_sel].iloc[0]["STOCK"]
    )
    prc_unit = float(
        df_inv_local[df_inv_local["CATEGORIA"] == prod_sel].iloc[0]["PRECIO"]
    )
  else:
    stk_max = 1
    prc_unit = 0.0

  cant_sel = col_add2.number_input(
      "Cant.",
      min_value=1,
      max_value=max(1, stk_max),
      value=1,
      key="modal_cant_prod",
  )
      ...
      for item in st.session_state["carrito"]:
        if item["producto"] == prod_sel:
          if item["cantidad"] + cant_sel <= stk_max:
            ...
            st.error(
                f"No hay suficiente stock. Máximo disponible: {stk_max}"
            )
            encontrado = True
          break
      if not encontrado:
  ...
  if not st.session_state["carrito"]:
    ...
    descuento = st.number_input(
        "🏷️ Descuento General ($)", min_value=0.0, max_value=100.0, value=0.0
    )
    total_final = max(0.0, subtotal - descuento)
    st.markdown(
        f'<div class="total-card">TOTAL A PAGAR: ${total_final:,.2f}</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("form_finalizar_venta"):
      ...
      if st.form_submit_button(
          "💰 FINALIZAR VENTA Y RECIBO", use_container_width=True
      ):
        for item in st.session_state["carrito"]:
          df_inv_local.loc[
              df_inv_local["CATEGORIA"] == item["producto"], "STOCK"
          ] -= item["cantidad"]

        guardar_csv_github(df_inv_local, FILE_INV)
        ...
        df_v_act = pd.concat(
            [st.session_state["df_ventas"], pd.DataFrame([nueva_v_dict])],
            ignore_index=True,
        )
        guardar_csv_github(df_v_act, FILE_VENTAS)
        st.session_state["df_ventas"] = df_v_act
        st.session_state["ultima_venta"] = nueva_v_dict
        st.session_state["carrito"] = []

        num_dest = NUMEROS_WHATSAPP[destino_recibo]
        st.session_state["redirect_url"] = generar_link_whatsapp(
            num_dest, nueva_v_dict
        )
        st.rerun()
...
with tab_venta:
  if df_inv.empty:
    ...
    camas_disponibles = df_inv[
        df_inv["CATEGORIA"].str.contains("CAMA", case=False, na=False)
        & (df_inv["STOCK"] > 0)
    ]["CATEGORIA"].tolist()

    colchones_disponibles = df_inv[
        df_inv["CATEGORIA"].str.contains("COLCHON|COLCHÓN", case=False, na=False)
        & (df_inv["STOCK"] > 0)
    ]["CATEGORIA"].tolist()
    ...
    if camas_disponibles and colchones_disponibles:
      ...
      sel_cama = c_combo1.selectbox(
          "🛏️ Seleccionar Cama", camas_disponibles, key="combo_cama_sel"
      )
      sel_colchon = c_combo2.selectbox(
          "💤 Seleccionar Colchón", colchones_disponibles, key="combo_colchon_sel"
      )

      prc_c = (
          float(df_inv[df_inv["CATEGORIA"] == sel_cama]["PRECIO"].values[0])
          if sel_cama
          else 0.0
      )
      prc_m = (
          float(df_inv[df_inv["CATEGORIA"] == sel_colchon]["PRECIO"].values[0])
          if sel_colchon
          else 0.0
      )
      precio_sugerido = prc_c + prc_m

      precio_combo = c_combo3.number_input(
          "💵 Precio Combo ($)",
          min_value=0.0,
      ...
      st.write("")
      if c_combo4.button(
          "⚡ AGREGAR COMBO", use_container_width=True, type="primary"
      ):
        if agregar_combo_al_carrito(sel_cama, sel_colchon, precio_combo):
          st.success(
              f"✅ Combo agregado al carrito: {sel_cama} + {sel_colchon}"
          )
          abrir_modal_venta()
    else:
      st.warning(
          "⚠️ Se requiere tener disponible al menos 1 Cama y 1 Colchón en stock"
          " para habilitar la creación de combos."
      )
    ...
    vendibles = df_inv[
        df_inv["CATEGORIA"].apply(lambda x: producto_es_vendible(df_inv, x))
    ].copy()
    ...
    cant_items_cart = sum(
        [item["cantidad"] for item in st.session_state["carrito"]]
    )
    if st.button(
        f"🛒 VER CARRITO DE COMPRAS ({cant_items_cart} productos)",
        use_container_width=True,
        type="primary",
    ):
      abrir_modal_venta()
    ...
    if (
        "redirect_url" in st.session_state
        and st.session_state["redirect_url"]
    ):
      link_red = st.session_state["redirect_url"]
      st.session_state["redirect_url"] = None

      st.markdown(
          f'<meta http-equiv="refresh" content="0; url={link_red}">',
          unsafe_allow_html=True,
      )
      st.success("✅ Venta procesada correctamente. Redirigiendo a WhatsApp...")
      st.markdown(
          f'<a href="{link_red}" target="_blank"'
          ' style="text-decoration:none;"><button style="width:100%;'
          " background-color:#25D366; color:white; border:none; padding:15px;"
          " border-radius:10px; font-weight:bold; cursor:pointer;">📲 SI NO SE"
          " ABRE AUTOMÁTICAMENTE, HAZ CLIC AQUÍ</button></a>",
          unsafe_allow_html=True,
      )

    if st.session_state.get("ultima_venta") and not st.session_state.get(
        "redirect_url"
    ):
      st.markdown("---")
      ...
      with c_w1:
        ...
        st.markdown(
            f'<a href="{link_w1}" target="_blank"'
            ' style="text-decoration:none;"><button style="width:100%;'
            " background-color:#25D366; color:white; border:none; padding:12px;"
            " border-radius:10px; font-weight:bold; cursor:pointer;">📲 REENVIAR"
            " A 0990847819</button></a>",
            unsafe_allow_html=True,
        )
      ...
      with c_w2:
        ...
        st.markdown(
            f'<a href="{link_w2}" target="_blank"'
            ' style="text-decoration:none;"><button style="width:100%;'
            " background-color:#25D366; color:white; border:none; padding:12px;"
            " border-radius:10px; font-weight:bold; cursor:pointer;">📲 REENVIAR"
            " A 0983576800</button></a>",
            unsafe_allow_html=True,
        )
      ...
      if HAS_REPORTLAB:
        ...
        st.download_button(
            ...
            data=pdf_buf,
            file_name=f"recibo_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
...
with tab_apartado:
  ...
  with col_ap1:
    ...
    if prods_apartado:
      prod_ap = st.selectbox(
          "📦 Seleccionar Producto", prods_apartado, key="sel_ap_prod"
      )
      ...
      with st.form("form_apartado"):
        ...
        cant_ap = st.number_input(
            "🔢 Cantidad", min_value=1, max_value=max(1, stk_ap), value=1
        )
        abono_ap = st.number_input(
            "💵 Abono Inicial ($)", min_value=0.0, value=10.0
        )
        ...
        st.markdown(
            f'<div class="total-card">Total: ${tot_ap:,.2f} | Abono:'
            f" ${abono_ap:,.2f} | Saldo: ${saldo_ap:,.2f}</div>",
            unsafe_allow_html=True,
        )

        if st.form_submit_button(
            "💾 GUARDAR APARTADO", use_container_width=True
        ):
          if not cli_ap.strip():
            ...
            est_ap = (
                "Pagado y Entregado" if saldo_ap <= 0 else "Apartado (Pendiente)"
            )

            df_inv.loc[df_inv["CATEGORIA"] == prod_ap, "STOCK"] -= cant_ap
            guardar_csv_github(df_inv, FILE_INV)
            ...
            df_v_act = pd.concat(
                [st.session_state["df_ventas"], pd.DataFrame([nuevo_ap])],
                ignore_index=True,
            )
            guardar_csv_github(df_v_act, FILE_VENTAS)
            st.session_state["df_ventas"] = df_v_act
  ...
  with col_ap2:
    ...
    if df_pending.empty:
      ...
      opciones_p = [
          f"ID {idx}: {row['CLIENTE']} - {row['CATEGORIA']} (Saldo:"
      ...
      st.write(
          f"**Cliente:** {row_sel['CLIENTE']} | **Teléfono:**"
          f" {row_sel['TELEFONO']}"
      )
      st.write(
          f"**Total Deuda:** ${row_sel['TOTAL']:,.2f} | **Saldo Restante:**"
          f" ${row_sel['SALDO_PENDIENTE']:,.2f}"
      )

      nuevo_abono = st.number_input(
          "💵 Monto a Abonar ($)",
          min_value=0.01,
          max_value=float(row_sel["SALDO_PENDIENTE"]),
          value=float(row_sel["SALDO_PENDIENTE"]),
      )
      ...
      if st.button("🤝 REGISTRAR ABONO", use_container_width=True):
        ...
        guardar_csv_github(df_ventas, FILE_VENTAS)
...
with tab_inventario:
  ...
  if (
      not df_inv.empty
      and "STOCK_MINIMO" in df_inv.columns
      and "STOCK" in df_inv.columns
  ):
    agotados = df_inv[df_inv["STOCK"] <= df_inv["STOCK_MINIMO"]]
    if not agotados.empty:
      st.warning(
          "⚠️ **Atención: Productos bajo el Stock Mínimo Recomendado**"
      )
      st.dataframe(
          agotados[["CATEGORIA", "STOCK", "STOCK_MINIMO"]],
          hide_index=True,
          use_container_width=True,
      )

  if (
      st.text_input("🔐 Clave Administrador", type="password", key="pwd_inv")
      == CLAVE_ADMIN
  ):
    principales = [
        n for n in df_inv["CATEGORIA"].tolist() if " - " not in str(n)
    ]
    opciones = [
...
with tab_historial:
  ...
  if not df_ventas.empty:
    csv_data = df_ventas.to_csv(index=False, encoding="utf-8-sig").encode(
        "utf-8-sig"
    )
    st.download_button(
    ...
    st.markdown(
        "#### 🔍 Selecciona con el check (☑️) las ventas que deseas eliminar:"
    )
    ...
    with col_b1:
      st.info(
          "📌 Registros marcados para eliminar:"
          f" **{cant_seleccionada}**"
      )

    with col_b2:
      pwd_borrado = st.text_input(
          "🔐 Clave Administrador para borrar",
          type="password",
          key="pwd_borrar_sel",
      )
    ...
    with c_btn1:
      if st.button(
          "🗑️ ELIMINAR VENTAS SELECCIONADAS",
          use_container_width=True,
          disabled=(cant_seleccionada == 0),
      ):
        if pwd_borrado == CLAVE_ADMIN:
          df_filtrado = df_editado[df_editado["SELECCIONAR"] == False].drop(
              columns=["SELECCIONAR"]
          )
          guardar_csv_github(df_filtrado, FILE_VENTAS)
          st.session_state["df_ventas"] = df_filtrado
          st.session_state["ultima_venta"] = None
          st.success(
              f"✅ Se eliminaron {cant_seleccionada} registro(s) correctamente."
          )
          st.rerun()
        else:
    ...
    with c_btn2:
      if st.button(
          "🔥 BORRAR TODO EL HISTORIAL COMPLETO", use_container_width=True
      ):
        if pwd_borrado == CLAVE_ADMIN:
          df_v_vacio = pd.DataFrame(columns=COLUMNAS_VENTAS)
          guardar_csv_github(df_v_vacio, FILE_VENTAS)
