...
# ============================================================
#            PERSISTENCIA EN GITHUB Y FUNCIONES AUXILIARES
# ============================================================
...
def guardar_csv_github(df, ruta_archivo):
    """Guarda localmente y hace Commit automático a GitHub para evitar la pérdida de datos."""
    # 1. Guardado local inmediato
    df.to_csv(ruta_archivo, index=False, encoding="utf-8-sig")

    # 2. Guardado en GitHub si existe el TOKEN en Secrets
    token = st.secrets.get("GITHUB_TOKEN", None)
    if not token:
        return

    url = f"https://api.github.com/repos/{REPO_GITHUB}/contents/{ruta_archivo}"
    ...
    try:
        ...
        payload = {
            "message": f"Actualización automática de {ruta_archivo}",
            "content": content_b64,
        }
        if sha:
            payload["sha"] = sha
...
def cargar_csv_github(ruta_archivo):
    """Lee el CSV desde GitHub para tener siempre los datos actualizados sin caché efímero."""
    token = st.secrets.get("GITHUB_TOKEN", None)
    if token:
...
def cargar_inventario():
    ...
    guardar_csv_github(df, FILE_INV)
    return df

def cargar_ventas():
    df = cargar_csv_github(FILE_VENTAS)
    df = normalizar_ventas(df)
    guardar_csv_github(df, FILE_VENTAS)
    return df
...
with tab_venta:
    if df_inv.empty:
        ...
        if "redirect_url" in st.session_state and st.session_state["redirect_url"]:
            ...
            # CORRECCIÓN DE SYNTAXERROR EN BOTÓN HTML:
            st.markdown(f'''
                <a href="{link_red}" target="_blank" style="text-decoration:none;">
        ...
        if st.session_state.get("ultima_venta") and not st.session_state.get("redirect_url"):
            ...
            with c_w1:
                ...
                st.markdown(f'''
                    <a href="{link_w1}" target="_blank" style="text-decoration:none;">
            ...
            with c_w2:
                ...
                st.markdown(f'''
                    <a href="{link_w2}" target="_blank" style="text-decoration:none;">
                        <button style="width:100%; background-color:#25D366; color:white; border:none; padding:12px; border-radius:10px; font-weight:bold; cursor:pointer;">
                            📲 REENVIAR A 0983576800
                        </button>
                    </a>
                ''', unsafe_allow_html=True)
